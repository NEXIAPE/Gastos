...
def _best_odds() -> pd.DataFrame:
    """Mejor cuota (máxima) por fixture/market/selection entre todos los libros."""
    sql = """
        SELECT fixture_id, market, selection, MAX(odd) AS odd
        FROM odds
...
def detect_value_bets(min_ev: float | None = None,
    ...
    """Detecta y devuelve todas las value bets con EV > min_ev."""
    ...
    for _, fx in fixtures.iterrows():
        home_r = ratings.get(int(fx["home_team_id"]))
        away_r = ratings.get(int(fx["away_team_id"]))
    ...
    with session() as conn:
        ...
        for b in bets:
            conn.execute(
                ...
                " ev, stake_pct, stake_amount) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (b.fixture_id, b.market, b.selection, b.model_prob, b.odd,
                 b.implied_prob, b.ev, b.stake_pct, b.stake_amount),
            )


def value_bets_dataframe(bets: list[ValueBet]) -> pd.DataFrame:
    return pd.DataFrame([asdict(b) for b in bets])
